...
void main()
{
    ...
    tex_uv = TexCoord;
    video_and_texture_size_inv = float4(1.0, 1.0, 1.0, 1.0) / 
                                 float4(params.video_size, params.texture_size);
    /* HSM Removed                  
    output_size_inv = float2(1.0, 1.0) / 
                      params.output_size;
    */
...
void main()
{
    ...
    if(aa_level > 0.5 && (global.hmss_curvature_mode > 0.5 || any(bool2((geom_overscan.x != 1.0), (geom_overscan.y != 1.0)))))
    {
